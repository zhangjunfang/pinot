...
public class PinotTenantRestletResource {
  ...
  private SuccessResponse createTenant(Tenant tenant) {
  ...
  @Consumes(MediaType.APPLICATION_JSON)
  private SuccessResponse updateTenant(Tenant tenant) {
    PinotResourceManagerResponse response;
    switch (tenant.getTenantRole()) {
      case BROKER:
  ...
  public String changeTenantState(
    ...
    try {
      ...
      throw new WebApplicationException("Error during " + type + " operation", Response.Status.INTERNAL_SERVER_ERROR);
